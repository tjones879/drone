...
public:
    ...
    size_t pull(uint8_t *dstBuff, size_t maxBufferSize, size_t waitTime=0);
...
private:
    ...
    BinarySemaphore w_mutex;
    BinarySemaphore r_mutex;
};
...
size_t StaticMessageBuffer::push(std::array<uint8_t, buffLen> srcBuffer)
{
    ...
    auto size = xMessageBufferSend(message_buffer, (void *)srcBuffer.data(), buffLen, portMAX_DELAY);
    w_mutex.give();
    return size;
}